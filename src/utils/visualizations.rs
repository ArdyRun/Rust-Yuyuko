...
use charts_rs::{svg_to_png, BarChart, Box as ChartBox, THEME_DARK};
...
pub fn generate_heatmap(
    ...
    for (row, day_name) in DAYS.iter().enumerate() {
        ...
        draw_text_mut(
            &mut img, GRAY_COLOR, 20, y as i32, day_scale, &font, day_name,
        );
    }
    ...
    for (i, color) in LEGEND_COLORS.iter().enumerate() {
        ...
        for dx in 0..14 {
            for dy in 0..14 {
                img.put_pixel(box_x + dx, legend_y + dy, *color);
            }
        }
    }
    draw_text_mut(
...
pub fn generate_bar_chart(
    ...
    // Create series
    let series_data: Vec<(String, Vec<f32>)> = data
        .iter()
    ...
    let mut bar_chart = BarChart::new_with_theme(
        series_data
            .into_iter()
            .map(|(name, values)| (name.as_str(), values).into())
            .collect(),
    ...
    let png_data = svg_to_png(&svg).map_err(|e| format!("PNG conversion failed: {:?}", e))?;
