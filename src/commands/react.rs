...
use crate::utils::emojis::{get_emoji_by_id, EMOJIS};
...
pub async fn react(
    ctx: Context<'_>,
    #[description = "ID atau link pesan yang ingin direact"] pesan: String,
) -> Result<(), Error> {
    ctx.defer_ephemeral().await?;

    // Parse message link or ID
    let (channel_id, message_id): (u64, u64) =
        if let Some((ch_id, msg_id)) = parse_message_link(&pesan) {
            (ch_id, msg_id)
        } else {
    ...
    if message_id == 0 {
        ctx.say("ID pesan tidak valid. Gunakan ID 17-19 digit atau link pesan.")
            .await?;
        return Ok(());
    }
        ...
        .await
    {
        Ok(ch) => ch,
        Err(_) => {
            ctx.say("Channel tidak ditemukan atau bot tidak memiliki akses.")
                .await?;
            return Ok(());
        }
    };
    ...
    let guild_channel = match channel.guild() {
        Some(gc) => gc,
        None => {
            ctx.say("Command ini hanya bisa digunakan di server.")
                .await?;
            return Ok(());
        }
    };
    ...
    // Fetch message
    let message = match guild_channel
        .message(ctx.http(), serenity::MessageId::new(message_id))
        .await
    {
        Ok(msg) => msg,
        Err(_) => {
            ctx.say(format!("Pesan tidak ditemukan di <#{}>.", channel_id))
                .await?;
            return Ok(());
        }
    };
    ...
    if age > 14 * 24 * 60 * 60 {
        ctx.say("Pesan terlalu lama (lebih dari 14 hari) untuk direact.")
            .await?;
        return Ok(());
    }
        ...
        if custom_id.starts_with("react_") {
            ...
            if parts.len() >= 2 {
                ...
                match message.react(ctx.http(), reaction).await {
                    Ok(_) => {
                        let emoji_name =
                            get_emoji_by_id(emoji_id).map(|e| e.name).unwrap_or("emoji");

                        let success_embed = serenity::CreateEmbed::new()
                            .title("React Berhasil")
                            .description(format!(
                                "Pesan berhasil direact dengan emoji **{}**",
                                emoji_name
                            ))
                            .color(0x00FF00)
                            .image(format!(
