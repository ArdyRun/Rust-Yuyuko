...
pub async fn send_quiz_selector(http: &serenity::Http, channel_id: serenity::ChannelId) -> Result<(), Error> {
    ...
    channel_id.send_message(http, serenity::CreateMessage::new()
        .embed(embed)
        .components(vec![row])
    ).await?;
    ...
    if let Some(gc) = channel {
        ...
        if let Some(cat_id) = category_id {
            ...
            if let Some(config) = data.guild_configs.get(&guild_id) {
                if let Some(selector_id) = &config.quiz_channel_id {
                    if gc.id.to_string() == *selector_id {
                        ctx.say("Cannot delete main selector channel (Protected via Config).").await?;
                        return Ok(());
                    }
                }
            }
            ...
            if let Err(e) = gc.delete(&ctx.http()).await {
             ...
             ctx.say("This command can only be used in quiz channels.").await?;
        }
      } // closing category_id
    } else {
        ctx.say("This command must be used in a guild channel.").await?;
    }
