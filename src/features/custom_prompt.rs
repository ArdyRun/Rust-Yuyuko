...
// Rate limit storage
static RATE_LIMITS: Lazy<RwLock<HashMap<u64, RateLimitEntry>>> = Lazy::new(|| {
...
pub fn is_rate_limited(user_id: u64) -> Result<bool, u64> {
    ...
    if let Some(entry) = limits.get_mut(&user_id) {
        if now.duration_since(entry.timestamp) > RATE_LIMIT_WINDOW {
            ...
            // Rate limited
            let time_left = RATE_LIMIT_WINDOW.as_secs() - now.duration_since(entry.timestamp).as_secs();
            Err(time_left)
        } else {
            entry.count += 1;
            Ok(false)
...
pub fn get_user_custom_prompt(user_id: u64) -> Option<String> {
    ...
    match fs::read_to_string(&path) {
        Ok(content) => {
            match serde_json::from_str::<UserPromptData>(&content) {
...
pub async fn fetch_prompt_from_rentry(client: &reqwest::Client, rentry_url: &str) -> Result<String> {
    let code = extract_rentry_code(rentry_url)
        .ok_or_else(|| anyhow::anyhow!("Invalid Rentry URL"))?;
...
pub fn validate_prompt_content(content: &str) -> Result<()> {
    ...
    let harmful_patterns = ["eval(", "Function(", "setTimeout(", "setInterval(", "require("];
